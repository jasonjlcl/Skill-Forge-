...
const envSchema = z.object({
  ...
  OTEL_EXPORTER_MODE: z.enum(['none', 'console', 'otlp']).default('none'),
  OTEL_SERVICE_NAME: z.string().default('skillforge-api'),
  OTEL_SERVICE_VERSION: z.preprocess(emptyStringToUndefined, z.string().optional()),
  OTEL_EXPORTER_OTLP_ENDPOINT: z.preprocess(emptyStringToUndefined, z.string().url().optional()),
  OTEL_EXPORTER_OTLP_TRACES_ENDPOINT: z.preprocess(emptyStringToUndefined, z.string().url().optional()),
  OTEL_EXPORTER_OTLP_METRICS_ENDPOINT: z.preprocess(emptyStringToUndefined, z.string().url().optional()),
  ...
  RETENTION_JOB_EDGE_SHARED_KEY: z.preprocess(emptyStringToUndefined, z.string().optional()),
...
export const env = {
  ...
  otelExporterMode: parsedData.OTEL_EXPORTER_MODE,
  otelServiceName: parsedData.OTEL_SERVICE_NAME,
  otelServiceVersion: parsedData.OTEL_SERVICE_VERSION,
  otelExporterOtlpEndpoint: parsedData.OTEL_EXPORTER_OTLP_ENDPOINT,
  otelExporterOtlpTracesEndpoint: parsedData.OTEL_EXPORTER_OTLP_TRACES_ENDPOINT,
  otelExporterOtlpMetricsEndpoint: parsedData.OTEL_EXPORTER_OTLP_METRICS_ENDPOINT,
  ...
  retentionJobAllowedServiceAccounts: (parsedData.RETENTION_JOB_ALLOWED_SERVICE_ACCOUNTS ?? '')
    ...
    .filter(Boolean),
  retentionJobEdgeSharedKey: parsedData.RETENTION_JOB_EDGE_SHARED_KEY,
